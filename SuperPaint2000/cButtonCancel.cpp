...
cButtonCancel::cButtonCancel(cWindowManager* _window, sf::Vector2f _size) : cButton(_size) {
    ...
    this->m_Text->setString("Cancel");
    this->m_Text->setFillColor(sf::Color::Black);
    this->m_Text->setOrigin(this->m_Text->getGlobalBounds().getCenter());
}

cButtonCancel::~cButtonCancel() {

}

void cButtonCancel::DrawButton(sf::Vector2f _pos) {
    // Sets Position
...
void cButtonCancel::UseButton() {
    this->m_Window->ShouldApplyColor = false;

    // If Color Picker FG WIndow is Open
    if (this->m_Window->m_ColorPickerFGWindow.isOpen() == true) {
        // Closes it without changing Color
...
void cButtonCancel::Select() {} // Removes Select() Functionality

void cButtonCancel::Unselect() {} // Removes Unselect() Functionality