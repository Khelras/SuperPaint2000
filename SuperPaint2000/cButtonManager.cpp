...
cButtonManager::cButtonManager(cToolManager* _toolManager, cWindowManager* _window) {
	...
	this->m_ButtonCircle = new cButtonCircle(_toolManager, sf::Vector2f(40, 125), sf::Color::Green);
}

cButtonManager::~cButtonManager() {
	delete(this->m_ButtonRectangle);
	delete(this->m_ButtonCircle);
}

void cButtonManager::DrawButtons() {
	this->m_Window->m_ToolWindow.draw(this->m_ButtonRectangle->m_ButtonShape);
	this->m_Window->m_ToolWindow.draw(this->m_ButtonCircle->m_ButtonShape);
}
...
void cButtonManager::ClickButtons(sf::Vector2i _mousePos) {
	...
	else if (this->m_ButtonCircle->m_ButtonShape.getGlobalBounds().contains(sf::Vector2f(_mousePos))) {
		this->m_ButtonCircle->UseButton();
	}
}
